...
* @hint 
*/
component extends="foundry.foundry" {
	...
	module.exports.beautifyError = function beautifyError(err, pathname) {
	  var message    = (err.message || err.toString()) + ' (in ' + pathname + ')',
	      beautified = new Error(message);

	  // provide reference to original error
	  beautified.originalError = err;
